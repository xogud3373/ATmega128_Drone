...
#define MAX_OCR 3690   // 2ms
#define MIN_OCR 1843   // 1ms
...
void MPU9250_Init(void)
{
    ...
    TWI_Write(GYRO_CONFIG, 0x01); // +250dps, DLPF-3600Hz-0.11ms
...
{
    *g_x = (float)(gyro_x - baseGyX) / 131; //각속도 변환 131
...
{ 
  ...
  TCNT1 = 0x00;
  ICR1 = 4608;  // TOP값  2500us = 2.5ms = 400Hz
...
//    int OCR3BW;
    ...
    OCR1A = 2500 - roll_output + pitch_output;
    moterOCR3B = (2500 + roll_output + pitch_output);
    
    OCR1B = 2500 - roll_output - pitch_output;
    moterOCR1C = (2500 + roll_output - pitch_output);
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if(timeflag_2ms)
        {
            sprintf(msg,"%.3f %.3f %d %d %.3f %.3f\n", pitch_kp, pitch, OCR1A,OCR1B,moterOCR1C,moterOCR3B); 
            puts_USART1(msg);   //문자열 msg1출력
            timeflag_2ms = 0;
            time_count = 0;  
        }
        
        //sprintf(msg,"1motor %d Pitch %.3f \n",OCR1A, pitch); 
        //puts_USART1(msg);   //문자열 msg1출력
