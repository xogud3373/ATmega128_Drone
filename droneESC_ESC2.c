...
void MPU9250_Init(void)
{
    ...
    TWI_Write(GYRO_CONFIG, 0x00); // +250dps, DLPF-3600Hz-0.11ms
...
{
    *g_x = (float)(gyro_x - baseGyX) / 131.072; //각속도 변환 131
    *g_y = (float)(gyro_y - baseGyY) / 131.072;
    *g_z = (float)(gyro_z - baseGyZ) / 131.072;
}
...
interrupt [TIM1_OVF] void tim1_overflow(void)
{
    if(power_flag)
    {
        if(i >= 1200)
        {   
           OCR1A = 2200 + (10*p_count);
           OCR1B = 2200 + (10*p_count);
           OCR1CH = (2200 + (10*p_count)) >> 8;
           OCR1CL = (2200 + (10*p_count)) & 0xff;
           i = 0;
        }
...
interrupt [TIM3_OVF] void tim3_overflow(void)
{
    if(power_flag)
    {
        if(j >= 1200)
        {   
            OCR3BH = (2200 + (10*p_count)) >> 8;
            OCR3BL = (2200 + (10*p_count)) & 0xff;
            j = 0;
...
void main(void)
{
    ...
    while(1)
    {
        ...
        if(timeflag_2ms)
        {
            sprintf(msg,"%.3f %.3f %.3f %.3f %.3f %.3f %.3f %d\n", a_x, a_y, a_z, g_x, g_y, g_z, pitch, OCR1A); 
